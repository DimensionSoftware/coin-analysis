...
module.exports = class Bot {
  ...
  async analyzeCurrencyPair(currencyPair) {
    ...
    let means = await this.loadMeanValues(currencyPair)
    ...
    let r = {
      sufficientVolume:                analysis.sufficientVolume({ candles }),
      anomalousVolumeAndPriceIncrease: analysis.anomalousVolumeAndPriceIncrease({ candles, means })
    }

    if (r.anomalousVolumeAndPriceIncrease.hit && r.sufficientVolume.hit) {
      let r2 = await this.send('bittrex', r.anomalousVolumeAndPriceIncrease.message)
    }
  ...
  async send(channel, message) {
    console.log('%s | %s | %s', moment().toLocaleString(), channel, message)
    return 1
  }
};
