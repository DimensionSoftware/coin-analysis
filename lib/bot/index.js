  ...
  async analyzeCurrencyPair(currencyPair, opts={}) {
    ...
    try {
      ...
      if (!opts.force) {
        ...
        if (lastSeen &&
            (lastSeen.equals(currentCandle) ||
             lastSeen.get('time').toString() == currentCandle.get('time').toString()))
        {
      ...
      const means = await this.loadMeanValues(currencyPair, options)
      const exchange = 'bittrex'
      const deviations = await this.loadStdDev(currencyPair, options)
      ...
      if (r.anomalousVolumeAndPriceIncrease.hit && r.previouslyLowVolume.hit) {
