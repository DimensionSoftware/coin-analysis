  ...
  async analyzeCurrencyPair(currencyPair, opts={}) {
    ...
    try {
      ...
      if (currentMinute === 30 || currentMinute === 0) {
        const
          past       = moment(currentCandle.time).subtract(96, 'hours'),
          candles30m = await db.bittrex.summarizePrices(currencyPair, '30min', { from: past }),
          macd       = await analysis.macdCrossUp({ candles: candles30m, below: true, above: true })

        r.macd = macd
        if (macd.hit) {
          let message = `${currencyPair} 30min MACD cross up ${h.urls.bittrex.market(currencyPair)}`
