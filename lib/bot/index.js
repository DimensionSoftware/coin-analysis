...
module.exports = class Bot {
  ...
  async analyzeCurrencyPair(currencyPair, opts={}) {
    ...
    try {
      ...
      const
        exchange      = 'bittrex',
        means         = await this.loadMeanValues(currencyPair, options),
        marketSummary = await bittrex.rest.getMarketSummary(currencyPair),
        deviations    = await this.loadStdDev(currencyPair, options)
