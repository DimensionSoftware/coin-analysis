  ...
  async analyzeCurrencyPair(currencyPair, opts={}) {
    ...
    try {
      ...
      if (!opts.force) {
        ...
        let lastSeenTime  = lastSeen.get('time').toString()
        let currentTime   = currentCandle.get('time').toString()
        if (lastSeen &&
            (lastSeen.equals(currentCandle) || lastSeenTime == currentTime))
        {
      ...
      if (r.anomalousVolumeAndPriceIncrease.hit && r.previouslyLowPriceVelocity.hit && r.sufficientVolume.hit) {
