...
const getCandles = Promise.promisify(bittrex.getcandles)
...
function normalize(result) {
  return {
    ...
    low:        result.L,
    volume:     result.V,
    baseVolume: result.BV,
    time:       moment(result.T),
  }
}
      ...
      while (self.go) {
        for (let m of state.markets) {
          ...
          try {
            ...
            events.emit('data', m.MarketName, normalize(tick.result[0]))
            winston.info({ currencyPair: m.MarketName, close: tick.result[0].C })
