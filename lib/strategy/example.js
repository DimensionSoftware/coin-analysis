...
module.exports = class Example extends Strategy {
   ...
   * @param {Moment} opts.startDate
   */
  async init({exchange, currencyPair, startDate}) {
    let d    = db[exchange]
    let past = startDate.clone().subtract(20, 'days')
    let r    = await d.summarizePrices(currencyPair, '30min', past, startDate)
