...
const analysis = {
  ...
  anomalousVolumeAndPriceIncrease({ exchange, currencyPair, candles, means }) {
    ...
    const candleHeightChange = h.percentChange(candleHeight, means.meanCandleHeight)
    const baseVolumeChange = h.percentChange(last.baseVolume, means.meanVolume.bv)
